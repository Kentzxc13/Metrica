...
export function AddPaymentModal({ isOpen, onClose, onAddTransaction }: AddPaymentModalProps) {
    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        const newTx: Transaction = {
            id: String(Date.now()),
            code: `#0${Math.floor(4914 + Math.random() * 100)}`,
            customer: newCustomer,
            product: newProduct || 'Standard SaaS License',
            status: 'Success',
            qty: 1,
            unitPrice: `$${Number(newAmount).toLocaleString()}`,
            totalRevenue: `$${Number(newAmount).toLocaleString()}`
        };
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-xs p-4">
            <div className="bg-white rounded-2xl max-w-md w-full p-6 shadow-2xl border border-gray-200 space-y-4 animate-in fade-in zoom-in-95">
                <div className="flex items-center justify-between border-b border-gray-100 pb-3">
                    <h3 className="text-base font-bold text-gray-900">Add New Transaction</h3>
                    <button
                        onClick={onClose}
                ...
                <form onSubmit={handleSubmit} className="space-y-3">
                    ...
                    <div>
                        <label className="block text-xs font-semibold text-gray-700 mb-1">Product / Plan</label>
                        <input
                            value={newProduct}
